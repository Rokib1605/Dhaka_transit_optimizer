...
vector<pair<double,double>> parse_lonlat_pairs(const string &line) {
    ...
    for (size_t i = 0; i + 1 < toks.size(); i += 2) {
        const string &lonS = toks[i];
        const string &latS = toks[i+1];
        if (lonS.empty() || latS.empty()) continue;
        try {
            double lon = stod(lonS);
            double lat = stod(latS);
...
int main() {
    ...
    if (!(cin >> src.first)) { cerr << "Bad input\n"; return 1; }
    cout << "Source Latitude = ";
    if (!(cin >> src.second)) { cerr << "Bad input\n"; return 1; }
    cout << "\n";
    cout << "Destination Longitude = ";
    if (!(cin >> dst.first)) { cerr << "Bad input\n"; return 1; }
    cout << "Destination Latitude = ";
    if (!(cin >> dst.second)) { cerr << "Bad input\n"; return 1; }
    cout << "\n";
    ...
    if (itSrc == coordToId.end()) {
        ...
        if (nearest == -1) { cerr << "Graph is empty, cannot connect source\n"; return 1; }
    ...
    if (itDst == coordToId.end()) {
        ...
        if (nearest == -1) { cerr << "Graph is empty, cannot connect destination\n"; return 1; }
    ...
    write_kml("Problem-2.kml", path, nodes);
    cout << "KML written to Problem-2.kml\n";
